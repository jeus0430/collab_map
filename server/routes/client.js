...
router.post("/login", async (req, res) => {
  ...
  const payload = {
    user: clients.users[0].username,
    password: clients.users[0].password,
    _id: clients.users[0]._id,
...
router.get("/create/session", async (req, res) => {
  ...
  const validUserIds = resp.users.map((v) => {
    return v._id
  })
  const validUserNames = resp.users.map((v) => {
    return v.username
  })
...
router.post("/create/instance", async (req, res) => {
  ...
  let updatedInstances = await Client.findOne(
    { _id: clientId },
    { inst: 1 }
  ).populate({
    path: "inst",
...
router.delete("/delete/instance", async (req, res) => {
  ...
  const resp = await InstModel.findOneAndDelete({
    instID: "4f81dc44-751f-4cfe-aaa2-9af9524608cb",
  })
