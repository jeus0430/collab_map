...
class Home extends React.Component {
  ...
  bindErasingAction(inst) {
    ...
    inst.line.on("mouseover", function (event) {
      ...
      if (that.state.erasing) {
        inst.line.setStyle({ opacity: 0.3 })
      }
    })
  ...
  createCursorControl(parent) {
    const cursorControler = L.Control.extend({
      ...
      onAdd: function () {
        ...
        btn.innerHTML = `<img src="` + PUBLIC_URL + `/assets/cursor-tool.svg">`
  ...
  createPenControl(parent) {
    const penControler = L.Control.extend({
      ...
      onAdd: function () {
        ...
        btn.innerHTML = `<img src="` + PUBLIC_URL + `/assets/pen-tool.svg">`
  ...
  createEraserControl(parent) {
    const eraserControler = L.Control.extend({
      ...
      onAdd: function () {
        ...
        btn.innerHTML = `<img src="` + PUBLIC_URL + `/assets/eraser-tool.svg">`
  ...
  createMarkerControl(parent) {
    const markerControler = L.Control.extend({
      ...
      onAdd: function () {
        ...
        btn.innerHTML = `<img src="` + PUBLIC_URL + `/assets/marker-tool.svg">`
  ...
  createPathControl(parent) {
    const pathControler = L.Control.extend({
      ...
      onAdd: function () {
        ...
        btn.innerHTML = `<img src="` + PUBLIC_URL + `/assets/path-tool.svg">`
  ...
  createAreaControl(parent) {
    const areaControler = L.Control.extend({
      onAdd: function () {
        ...
        btn.innerHTML = `<img src="` + PUBLIC_URL + `/assets/area-tool.svg">`
  ...
  componentDidMount() {
    ...
    this.followcursor = L.marker([0, 0], {
      pane: "overlayPane",
      interactive: false,
    }).addTo(this.map)

    this.followcursor.setOpacity(0)
    this.tooltip = this.followcursor
      .bindTooltip("", {
        permanent: true,
        offset: [5, 25],
        sticky: true,
  ...
  resetTool = () => {
    ...
    if (this.followcursor) this.followcursor.setLatLng([0, 0])
  }
  ...
  setMap = (map) => {
      ...
      if (this.state.linetool || this.state.areatool) {
        ...
        if (line.length > 5) {
          ...
          const reqs = await axios.post(
            "http://localhost:5000/collaborate/scribble",
            {
    ...
    map.addEventListener("mousemove", (event) => {
      ...
      if (this.state.areaon) {
        ...
        this.followcursor.setLatLng([lat, lng])
        this.followcursor.openTooltip()
        this.followcursor.setTooltipContent(
        ...
        if (e.shape == "Polygon") {
          ...
          if (e.layer._latlngs.length == 1) {
            ...
            e.layer._latlngs.forEach(function (coordinate, index) {
              if (index != 0) {
                linedistance +=
                  e.layer._latlngs[index - 1].distanceTo(coordinate)
              }
            })
            this.followcursor.setTooltipContent(
              linedistance / 1000 + "km | Double click to finish"
            )
  ...
  createMarkerDirectly(lat, lng, id) {
    ...
    if (typeof id === "undefined") id = this.getNewID()
  ...
  bindTooltiptoCurrentInst() {
    ...
    if (inst.type == "area") {
      ...
      inst.trigger.bindTooltip(
        "<h1>" +
          inst.name +
          "</h1><h2>" +
          inst.desc +
          '</h2><div class="shape-data"><h3><img src="assets/area-icon.svg">' +
          inst.area +
          ' km&sup2;</h3></div><div class="shape-data"><h3><img src="assets/perimeter-icon.svg">' +
          inst.distance +
          ' km</h3></div><div class="arrow-down"></div>',
        {
          permanent: false,
          direction: "top",
          interactive: false,
      ...
      inst.trigger.bindTooltip(
        "<h1>" +
          ...
          "</h1><h2>" +
          inst.desc +
          '</h2><div class="shape-data"><h3><img src="assets/distance-icon.svg">' +
          inst.distance +
          ' km</h3></div><div class="arrow-down"></div>',
        {
          permanent: false,
      ...
      inst.trigger.bindTooltip(
        "<h1>" +
          ...
          "</h1><h2>" +
          inst.desc +
          '</h2><div class="shape-data"><h3><img src="assets/marker-small-icon.svg">' +
          inst.lat.toFixed(5) +
          ", " +
          inst.lng.toFixed(5) +
